...
// Shared globally in the JS module context
let cachedBaseEpicycles = null;
let cachedBaseBounds = null;
let calculationPromise = null;

export function getBaseEpicycles(numSamples) {
...
// Get the epicycles asynchronously. Resolves instantly if already calculated.
// If not calculated yet, runs the calculation inside a setTimeout so it does not block the current stack.
export function getBaseEpicyclesAsync(numSamples) {
    if (cachedBaseEpicycles && cachedBaseEpicycles.length === numSamples) {
        return Promise.resolve({ epicycles: cachedBaseEpicycles, bounds: cachedBaseBounds });
    }
    
    if (calculationPromise) return calculationPromise;

    calculationPromise = new Promise(async (resolve) => {
        // Check localStorage first for instant reload!
        try {
            const cachedStr = localStorage.getItem(`fourier_epicycles_${numSamples}`);
            if (cachedStr) {
                const parsed = JSON.parse(cachedStr);
                cachedBaseEpicycles = parsed.epicycles;
                cachedBaseBounds = parsed.bounds;
                resolve({ epicycles: cachedBaseEpicycles, bounds: cachedBaseBounds });
                return;
            }
        } catch(e) {
        ...
        // Yield initially to allow the browser to paint the DOM components instantly
        ...
        // Sample points asynchronously in chunks to prevent freezing the page rendering
        const chunkSize = 128;
        for (let i = 0; i < numSamples; i++) {
            const pt = svgPath.getPointAtLength((i / numSamples) * totalLen);
            rawPoints.push(new Complex(pt.x, pt.y));
        ...
        try {
            localStorage.setItem(`fourier_epicycles_${numSamples}`, JSON.stringify({
                epicycles: cachedBaseEpicycles,
                bounds: cachedBaseBounds
            }));
        } catch(e) {
...
// Background pre-computation: start computing as soon as this module loads,
// deferred so it does not block the initial page paint thread.
if (typeof window !== 'undefined') {
    setTimeout(() => {
        try {
            getBaseEpicyclesAsync(2048);
        } catch (e) {
            console.warn("Pre-calculation failed, will compute on demand:", e);
        }
    }, 100);
}
